...
//----------------My Code--------------
    ...
    struct Arc : Shape {        //Lesson 13 Task 1
		Arc(Point p, int ww, int hh, int angle1, int angle2)	// center, min, and max distance from center
			:w(ww), h(hh), angl1(angle1), angl2(angle2) {
			if (angle1 < 0 || angle1 > angle2) error("angle1 error");
			if (angle2 > 360) error("angle2 error");
			angl1 = angle1;
			angl2 = angle2;
    ...
    struct Box : Shape {        //Lesson 13 Task 2

		Box(Point xy, int ww, int hh) : h(hh), w(ww)
		{
			add(xy);
		...
		int height() const { return h; }
		int width() const { return w; }
		void set_ratio(int a) {
			if (a > 2 * h || a > 2 * w) error("Bad set_ratio");
			r = a;
		}
		int ratio() { return r; }
	private:
    ...
    struct Arrow : Shape {      //Lesson 13 Task 3
		Arrow(Graph_lib::Point p1, Graph_lib::Point p2);
		void draw_lines() const;
	private:
		double k, f;	// y = k*x + f - line function
	};
    
    struct Hexagon : Shape {        //lesson 13 Task 8
		Hexagon(Graph_lib::Point pCentr, int r);
		void draw_lines() const;
	...
	struct Frowny : Circle {          //Lesson 14 Task 1A
		Frowny(Point p, int r)
			...
			add(Point(point(1).x + r / 4, point(1).y + r / 4));
		};
		void draw_lines() const;
	private:
		int _r;
